...
def generate_sheet(history, players, dealer_idx, current_picks, is_fin=False, status=""):
    ...
    calc_h = max(2200, 1100 + (num_rounds * 210))
    img = Image.new('RGB', (width, calc_h), (255, 255, 255))
    draw = ImageDraw.Draw(img)
    for y in range(100, calc_h, 80):
...
if cmd:
    ...
    if "winner" in raw:
        ...
        if wm:
            winner, new_r, t_lost = wm.group(1).capitalize(), {p: 0 for p in st.session_state.players}, 0
            for p_n, p_v in sc:
                p_n = p_n.capitalize()
                if p_n in new_r: new_r[p_n] = -int(p_v); t_lost += int(p_v)
            if winner in new_r:
                new_r[winner] = t_lost; st.session_state.history.append(new_r)
                st.session_state.dealer_idx = (st.session_state.dealer_idx + 1) % len(st.session_state.players)
                st.session_state.current_picks = {p:0 for p in st.session_state.players}
                st.session_state.redo_stack = []; set_status("Round recorded!"); save_checkpoint(); st.rerun()
    elif "dealer" in raw:
