...
# 2. PERSISTENCE (Lightweight to prevent loading errors)
...
if 'players' not in st.session_state:
    if not load_checkpoint():
        st.session_state.update({
            ...
            "dealer_idx": 0, "current_picks": {}, "last_msg": "", "msg_time": 0
        })
...
def generate_sheet(history, players, dealer_idx, current_picks, is_fin=False):
    width, height = max(1200, (len(players) + 1) * 200), max(2200, 1100 + (len(history) * 210))
